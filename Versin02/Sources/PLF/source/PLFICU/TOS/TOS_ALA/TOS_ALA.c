...
{
    (void)LDBStopTimer( cTOSTimerLdbDesignedForALA );
}
...
void TOSSetRelAlarm_Interne( tTOSAlarmType const pAlarm, const tTOSTickAlarm Cycle )
{
    ...
    if (pAlarm != NULL)
    {
        if (Cycle != (tTOSTickAlarm)0)
        {
            ...
            // Polyspace justification
            // Function GetSlot returns possition of less sungifician 1 in non zero U8 parameter
            // This possition is every case is less then cNbSlot equal to 8
...
void TOSCancelAlarm( const tTOSAlarm* const pAlarm )
{
    ...
    if (pAlarm != NULL)
    {
        if (pAlarm->Period != (tTOSTickAlarm)0)
        {

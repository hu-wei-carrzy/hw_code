...
#define mReadFlagStatusOk(hDbk)   ((Flags[(hDbk)] &   cMskFlagStatusOk) != 0U )

#define mSetFlagWriteMode(hDbk)   { Flags[(hDbk)] |=  cMskFlagWriteMode; }
#define mClearFlagWriteMode(hDbk) { Flags[(hDbk)] &= ~cMskFlagWriteMode; }
#define mReadFlagWriteMode(hDbk)  ((Flags[(hDbk)] &   cMskFlagWriteMode)!= 0U)
...
void DATDbkStartReadingAllBanks ( void )
{
    ...
    for ( HandleDbk = 0U ; HandleDbk < cNbDbk ; HandleDbk ++ )
    {
...
void DATDbkCallBackEep ( tDATEepProcessHandle ProcessHandle, BOOL CallBackStatusOk )
{
    ...
    case cGraphDbkWaitState:
        if ( mReadFlagWriteMode(HandleDbkInUse) != cFalse )
        {
            ...
            (void)mLIBmemcpy ( (void*)(DATEepBuffer + 1) , (void*)DATDbkTableBankRom [HandleDbkInUse].AddBlockRam , (SizeDbkChkInUse - 1) * 2 );
            // Calcule le checksum
    ...
    case cGraphDbkReadState:
        ...
        if ( SlaveStatus != cGraphResultProcessing )
        {
            ...
            // la taille en RAM est en mot (2 octects) et n'a pas de checksum
            // RAM size is in word and haven't checksum
            (void)mLIBmemcpy ( (void*)DATDbkTableBankRom [HandleDbkInUse].AddBlockRam , (void*)(DATEepBuffer + 1) , (SizeDbkChkInUse - 1) * 2 );
...
tGraphResult WritingDbkGraph ( void )
{
    ...
    case cGraphWriteDbkWaitState:
        ...
        else
        {
            // ecrit le bloc 1
            // write block 1
            DATEepWrite ( DATEepBuffer , 0U , SizeDbkChkInUse );
        }
        // va a l'etape suivante
        // go to next step
...
tGraphResult ReadingDbkGraph ( void )
{
    ...
    case cGraphReadDbkWaitState:
        ...
        // block 1 reading
        SlaveStatus = ReadingBlockGraph ( 0U );
        // va a l'etape suivante
        // go to next step
    ...
    case cGraphReadDbkBk1State:
        ...
        else if ( SlaveStatus == cGraphResultError )
        {
            ...
            // copy working RAM to bank RAM
        	 (void)mLIBmemcpy ( (void*)DATDbkTableBankRom [HandleDbkInUse].AddBlockRam , (void*)(DATEepBuffer + (U16)1U) , (SizeDbkChkInUse - 1) * 2 );
    ...
    case cGraphReadDbkBk2State:
        ...
        else if ( SlaveStatus == cGraphResultError )
        {
            ...
            (void)mLIBmemcpy ( (void*)(DATEepBuffer + (U16)1U) , (void*)DATDbkTableBankRom [HandleDbkInUse].AddBlockRam , (SizeDbkChkInUse - 1) * 2 );
...
void DATDbkInit (void)
{
    ...
    (void)mLIBmemset( Flags, 0U, sizeof(Flags) );
    (void)mLIBmemset( DbkError, 0U, sizeof(DbkError) );

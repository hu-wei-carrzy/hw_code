...
// Language:     C
// -----------------------------------------------------------------------------
// Component:  EEP state machine to control the EEP reading and writing
// -----------------------------------------------------------------------------
//******************************************************************************
// This line is used to separate code and file header.This is begin.
//*****************************BEGIN_NOMOVE*************************************
// Time:2016-09-19 11:06:41
// 1:enable EEPtask content because now EEP is available.
//2018年1月22日 10:51:57
//zjb
//修改tActiveProcessList类型为longlong
//*****************************END_NOMOVE***************************************
...
#if cDATEepExtendedNumberOfProcess <= 64U
typedef unsigned long long tActiveProcessList;
        #else
...
static void SendCommand( void )
{
    ...
    case( cCmdRead ) :
        ...
        LDBReceiveWithAddr( cLDBChannelLDB_EEP, mConvAdress16ToAdr8(Command.Adress), &Command.Msg );
        break;

    case( cCmdWrite ) :
        // This is a write command
        LDBSendWithAddr( cLDBChannelLDB_EEP, mConvAdress16ToAdr8(Command.Adress), &Command.Msg );
        break;

    case( cCmdLock ) :
        // This is a lock command
        LDBControl(cLDBChannelLDB_EEP, cLDBEepLock);
        break;

    case( cCmdUnlock ) :
        // This is an unlock command
        LDBControl(cLDBChannelLDB_EEP, cLDBEepUnLock );
        break;

#ifdef cLIBStkUnprotect_Used
    case( cCmdUnprotect ) :
        // This is an unprotect command
        LDBControl(cLDBChannelLDB_EEP, cLDBEepUnprotect );
        break;
...
static void EndOfCommand( void )
{
    ...
    case( cStateChecking  ) :
        ...
        if( mIsDataIdentical((&UserBuffer[OffsetFromUserBuffer]), 
						        ReadBuffer, Command.Msg.Lng) )
        {
            WriteRetryCounter = 0U;

            ReadDataLng = (tDATEepOffset)(Command.Msg.Lng / 2U);
...
static void CommandStateMachine(void)
{
    ...
    case(cCommandStateProcessing):
        ...
        {
            LDBControl(cLDBChannelLDB_EEP, cLDBEepCheckWriteCompletion);
        }
#endif
...
static void ScheduleProcess(void)
{
    ...
    if( ActiveProcessList != (tActiveProcessList) 0U )
    {
        ...
        ActiveProcessHandle = 0U;
        ActiveProcessMask   = 1U;
        while((ActiveProcessMask & ActiveProcessList) == (tActiveProcessList) 0U)
        {
            ActiveProcessHandle ++;
            ActiveProcessMask <<= 1U;
        }
